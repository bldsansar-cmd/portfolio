const commonTags_en = ['java', 'javascript', 'html', 'css', 'mysql'];

const message_en = {
	tagline: 'Writer, freelancer software engineer.',
	intro: 'I develop front-end to back-end program, sometimes write.',

	nav: {
		about: 'about',
		experience: 'experience',
		projects: 'projects',
	},

	sections: {
		about: 'About',
		experience: 'Experience',
		projects: 'Projects',
	},

	aboutText: [
		'Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae at nobis ipsam. Neque vel necessitatibus saepe exercitationem perferendis, quam earum esse quo rem fuga, quidem autem repudiandae molestiae sunt labore.',
		'Lorem, ipsum dolor sit amet consectetur adipisicing elit. Dolore, nostrum?',
		'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nam sapiente odit dignissimos culpa nisi dicta ratione quos atque dolores unde!',
	],

	footer: 'This site is developed with very simple html, css, javascript by front-end and Java springboot framework by back-end coding and deployed on tomcat server engine.',

	experience: [
		{
			period: '2000-2001',
			title: 'front-end engineer',
			detail: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores perspiciatis assumenda omnis hic voluptates minus harum quod, consequatur adipisci officiis eligendi, sed inventore nisi velit odio similique reiciendis provident in.',
			tags: commonTags_en,
		},
		{
			period: '2000-2001',
			title: 'back-end engineer',
			detail: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores perspiciatis assumenda omnis hic voluptates minus harum quod, consequatur adipisci officiis eligendi, sed inventore nisi velit odio similique reiciendis provident in.',
			tags: commonTags_en,
		},
		{
			period: '2000-2001',
			title: 'senior engineer and team leader',
			detail: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores perspiciatis assumenda omnis hic voluptates minus harum quod, consequatur adipisci officiis eligendi, sed inventore nisi velit odio similique reiciendis provident in.',
			tags: commonTags_en,
		},
		{
			period: '2000-2001',
			title: 'senior engineer and advisor.',
			detail: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores perspiciatis assumenda omnis hic voluptates minus harum quod, consequatur adipisci officiis eligendi, sed inventore nisi velit odio similique reiciendis provident in.',
			tags: commonTags_en,
		},
	],

	projects: [
		{
			title: 'ABCD web application',
			detail: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores perspiciatis assumenda omnis hic voluptates minus harum quod, consequatur adipisci officiis eligendi, sed inventore nisi velit odio similique reiciendis provident in.',
			tags: commonTags_en,
		},
		{
			title: 'QR code generator',
			detail: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores perspiciatis assumenda omnis hic voluptates minus harum quod, consequatur adipisci officiis eligendi, sed inventore nisi velit odio similique reiciendis provident in.',
			tags: commonTags_en,
		},
		{
			title: 'Find near hospital',
			detail: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores perspiciatis assumenda omnis hic voluptates minus harum quod, consequatur adipisci officiis eligendi, sed inventore nisi velit odio similique reiciendis provident in.',
			tags: commonTags_en,
		},
		{
			title: 'Near vegan (find nice vegan restaurant web service)',
			detail: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Asperiores perspiciatis assumenda omnis hic voluptates minus harum quod, consequatur adipisci officiis eligendi, sed inventore nisi velit odio similique reiciendis provident in.',
			tags: commonTags_en,
		},
	],
};
